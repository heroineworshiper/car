...
{
 	int result = (int)sin_table[x] * truck.motor[motor].power / MAX_SIN;
...
{
	motor_t *motor = &truck.motor[motor_number];
...
{
	int i;
	motor_t *motor = &truck.motor[number];
...
void handle_commutation(int motor_number)
{
	...
	if(truck.mode == COMMUTATING)
	{
		...
		int reverse = 0;
		if(truck.throttle_reverse) reverse = !reverse;
...
void handle_motors()
{
	...
	{
		motor_t *left_motor = &truck.motor[LEFT_MOTOR];
		motor_t *right_motor = &truck.motor[RIGHT_MOTOR];

		if(truck.throttle <= THROTTLE_MIN)
		{
// reset
			truck.mode = OFF;
			truck.period = MAX_PERIOD;
			left_motor->power = 0;
			right_motor->power = 0;
...
// test if steering
			if(truck.steering < STEERING_MID - STEERING_MAX)
			{
				phase = -FAST_STEERING;
				enabled = 1;
			}
			else
			if(truck.steering < STEERING_MID - STEERING_MIN)
			{
				phase = -SLOW_STEERING;
				enabled = 1;
			}
			...
			if(enabled)
			{
				...
				for(i = 0; i < MOTORS; i++)
				{
					truck.motor[i].power = truck.stepper_power;
					truck.period = STEERING_PERIOD;
					truck.motor[i].phase += phase * 
						truck.motor[i].stepper_direction * 
						truck.motor[i].turn_sign;
				}
			}

			TIM1->CCR1 = truck.period;
			
			write_motors();
		}
/*
 * 		else
 * // truck.throttle > THROTTLE_MIN
 * 		{
 * // acceleration phase
 * 			if(truck.mode == STEPPER)
 * 			{
 * 				int sign = truck.throttle_reverse ? -1 : 1;
 * 				for(i = 0; i < MOTORS; i++)
 * 				{
 * 					truck.motor[i].phase += MOTOR_STEP * 
 * 						sign * 
 * 						truck.motor[i].stepper_direction;
 * 					truck.motor[i].power = truck.stepper_power;
 * 				}
 ...
 * 
 * // acceleration done
 * 				if(truck.period <= MIN_STEPPER_PERIOD)
 * 				{
 * 					truck.mode = COMMUTATING;
 * 					left_motor->do_ramp = 1;
 * 					right_motor->do_ramp = 1;
 * 					left_motor->power = truck.operating_power0;
 * 					right_motor->power = truck.operating_power0;
...
// have to change the period here, so the update rate is constant
		...
		if(truck.mode == COMMUTATING)
		{
			for(i = 0; i < MOTORS; i++)
			{
				motor_t *motor = &truck.motor[i];
				if(motor->do_ramp && motor->power < truck.operating_power)
				{
					motor->power += 8;
				}

				if(motor->power > truck.operating_power)
				{
					motor->power = truck.operating_power;
					motor->do_ramp = 0;
				}
/*
 * if(i == 0)
...
void handle_battery()
{
	...
	{
		truck.operating_power = MAX_PWM * POWER_VOLTAGE / voltage;
	}
	else
	{
		truck.operating_power = MAX_PWM;
	}
	
	if(voltage > STEPPER_VOLTAGE)
	{
		truck.stepper_power = MAX_PWM * STEPPER_VOLTAGE / voltage;
	}
	else
	{
		truck.stepper_power = MAX_PWM;
	}

TRACE2
print_number(battery);
print_float(voltage);
print_number(truck.operating_power);
print_number(truck.stepper_power);
...
//			print_number(truck.commutations[0]);
//			print_number(truck.crossing_time[0]);
//			print_number(truck.period);
//			print_text("rpm=");
//			print_number(4000.0f * 930.0f / truck.period_lowpass);
...
//	TOGGLE_PIN(DEBUG_GPIO, DEBUG_PIN);
	...
	if((truck.motor[LEFT_MOTOR].adc->SR & ADC_FLAG_EOC) != 0)
	{
		handle_motor_adc(LEFT_MOTOR);
	}

	if((truck.motor[RIGHT_MOTOR].adc->SR & ADC_FLAG_EOC) != 0)
	{
...
// steering while driving
		if(truck.steering < STEERING_MID - STEERING_MAX)
		{
			if(truck.throttle_reverse)
				right_motor->power = truck.operating_power * STEERING_FRACTION / 100;
			else
				left_motor->power = truck.operating_power * STEERING_FRACTION / 100;
		...
		{
			left_motor->power = truck.operating_power;
			right_motor->power = truck.operating_power;
		}

		if(!manual_steering)
		{

			int result = do_pid(&truck.heading_pid, 
				-imu.current_heading, 
				-imu.gyro_z_centered / imu.angle_to_gyro / NAV_HZ);

			if(truck.throttle_reverse) result *= -1;
...
void TIM2_IRQHandler()
{
	if(TIM2->SR & TIM_FLAG_Update)
	{
		...
		TIM2->CCR1 = truck.motor[LEFT_MOTOR].pwm[0];
		TIM2->CCR2 = truck.motor[LEFT_MOTOR].pwm[1];
		TIM2->CCR3 = truck.motor[LEFT_MOTOR].pwm[2];
...
void TIM5_IRQHandler()
{
	if(TIM5->SR & TIM_FLAG_Update)
	{
		...
		TIM5->CCR1 = truck.motor[RIGHT_MOTOR].pwm[0];
		TIM5->CCR2 = truck.motor[RIGHT_MOTOR].pwm[1];
		TIM5->CCR3 = truck.motor[RIGHT_MOTOR].pwm[2];
...
int main(void)
{
	...
	truck.stepper_power = MAX_PWM * 50 / 100;
//	truck.stepper_power = MAX_PWM;
//	truck.operating_power0 = MAX_PWM;
//	truck.operating_power = MAX_PWM * 93 / 100;
	truck.operating_power = MAX_PWM;
	...
	for(i = 0; i < SENSORS; i++)
	{
		for(j = 0; j < MOTORS; j++)
		{
			truck.motor[j].adc_min[i] = 65536;
			truck.motor[j].adc_max[i] = 0;
		}
	}

//	truck.test_motors = 1;
	if(truck.test_motors)
	{
		truck.motor[LEFT_MOTOR].power = MAX_PWM;
		truck.motor[RIGHT_MOTOR].power = MAX_PWM;
	}
