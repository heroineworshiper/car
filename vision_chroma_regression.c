...
// tables for linear regression
// average of all mask pixels in a row
	float mask_x[vision.working_h];

	bzero(vision.mask, vision.working_w * vision.working_h);


// use linear regression of all masked pixels to get next key pixel
	for(key_y = vision.working_h - 1; key_y > vision.working_h / 2; key_y--)
//	for(key_y = vision.working_h - 1; key_y > vision.working_h - 3; key_y--)
	{
...
//printf("detect_path %d frame=%d key_y=%d\n", __LINE__, vision.frames_written, key_y);
		...
		{
// get linear regression line of all masked pixels
			float avg_mask_x = 0;
			float avg_mask_y = 0;
			int avg_mask_total = 0;
			for(i = vision.working_h / 2; i < vision.working_h; i++)
			{
				int accum = 0;
				int total = 0;
				for(j = 0; j < vision.working_w; j++)
				...
				if(total)
				{
					mask_x[i] = (float)accum / total;
					avg_mask_x += mask_x[i];
					avg_mask_y += i;
					avg_mask_total++;
//vision.mask[i * vision.working_w + (int)mask_x[i]] = 2;
				}
				else
				{
					mask_x[i] = (float)vision.working_w / 2;
				}
				
			}
			
			avg_mask_x /= avg_mask_total;
			avg_mask_y /= avg_mask_total;

//printf("avg_mask_x=%f avg_mask_y=%f\n", avg_mask_x, avg_mask_y);
// average of all mask pixels in a column
			float x_error_squared = 0;
			float error_product = 0;
			for(i = vision.working_h / 2; i < vision.working_h; i++)
			{
				float x_error = mask_x[i] - avg_mask_x;
				float y_error = i - avg_mask_y;
				error_product += x_error * y_error;
				x_error_squared += SQR(x_error);
			}
			
//printf("x_error_squared=%f error_product=%f\n", x_error_squared, error_product);
			if(x_error_squared != 0)
			{
				float slope = 0;
				slope = (float)error_product / x_error_squared;
// y = mx + b
// y - mx = b
				float intercept = avg_mask_y - slope * avg_mask_x;
// y = mx + b
// y - b = mx
// (y - b) / m = x
				key_x1 = (key_y - intercept) / slope;
//printf("chroma_key %d slope=%f intercept=%f\n", __LINE__, slope, intercept);
			}

//printf("chroma_key %d key_x1=%f key_y=%d\n", __LINE__, key_x1, key_y);
